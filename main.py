...
async def on_member_join( member ):
    channel = bot.get_channel(1027576029486792725)
    role = discord.utils.get( member.guild.roles, id = 1027575987422121985 )
...
# @bot.tree.command(name = 'isk', description = 'Подать судебный иск')
# async def state (interaction: discord.Interaction):
#    await interaction.response.send_message(f"{interaction.user.mention},")

# @bot.tree.command(name = 'prok', description = 'Обращение в Прокуратуру')
# async def state (interaction: discord.Interaction):
#    await interaction.response.send_message(f"{interaction.user.mention},")
